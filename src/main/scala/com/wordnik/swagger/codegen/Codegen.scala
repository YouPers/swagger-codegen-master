...
class Codegen(config: CodegenConfig) {
  ...
  final def writeSupportingClasses(
    ...
    /*//println("DEBUG JACOPO: " + apis+"\n")
    for ((key, value) <- apis) {
      println("DEBUG JACOPO KEY: " + key+"\n")
      println("DEBUG JACOPO VALUE: " + value+"\n")
      println("-------------------------\n")

      for ((key2, value2) <- value) {
        //println("List: " + key2+" |||| "+value2+"\n")
        println(value2.mobileSDK)
        //println(value2.disabled)
      }
      println("-------------------------\n")
     }*/
      ...
      apis.foreach(a => {
        apiList += Map(
          ...
          "operations" -> {
            (for (t <- a._2) yield
            {
              Map("operation" -> apiToMap(t._1, t._2), "path" -> t._1)
            }).toList
